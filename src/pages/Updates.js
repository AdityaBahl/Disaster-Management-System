    ...
    return (
      <div className="container flex flex-col items-center justify-center">
        ...
        <div className="custom-space row">
          {
            !this.state.loading && this.state.articles.map((element) => {
              return <div className="col-md-4 flex flex-col items-center mt-6 border-solid border-2 border-black w-96" key={element.url}>
        ...
        <div className="container mx-auto flex justify-between">
          <button  disabled={this.state.page <= 1} type="button" className="items-start btn btn-dark" onClick={this.handlePrevClick}>&larr; Previous</button>
          <button disabled={this.state.page + 1 > Math.ceil(this.state.totalResults / this.props.pageSize)} type="button" className="mr-0 btn btn-dark" onClick={this.handleNextClick}>Next &rarr;</button>
        </div>
      </div >
    )
  }
